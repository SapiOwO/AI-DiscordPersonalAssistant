...
* **No Unmanaged Audio Leaks**: Never synthesized or transcode audio without using a clean `try...finally` block. All temporary file handles in `temp_audio/` must be explicitly closed and unlinked (deleted) to prevent running out of disk space on the host machine.
